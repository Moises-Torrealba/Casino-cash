...
export default class Cl_vGeneral {
  ...
  crearHTMLElement(
    ...
    if (!domElement) {
      ...
      console.error(msg);
      // Mantener el comportamiento de fallo para no ocultar errores, pero dejar rastro en la consola
      alert(msg);
      throw new Error(msg);
    }
    // Attach handlers and expose refresh for debugging
    console.debug(`crearHTMLElement: asignando handlers a ${domElementName}`);
